...
namespace Rage_of_the_Dark_Lord.SpritesClass.Map
{
    class Stairs
    {
       ...
       static public bool climbStairs = false;
        ...
        {
            listStairs.Insert(0, new Stairs(new Rectangle(1842, 272, 150, 210), new Texture2D(graphics.GraphicsDevice, 100, 100)));
            listStairs.Insert(1, new Stairs(new Rectangle(1842, 178, 150, 210), new Texture2D(graphics.GraphicsDevice, 100, 100)));
            listStairs.Insert(2, new Stairs(new Rectangle(1842, 84, 150, 210), new Texture2D(graphics.GraphicsDevice, 100, 100)));//Y=-94
        }
        public void Draw(SpriteBatch spriteBatch)
        {
            ...
            spriteBatch.Draw(listStairs[2].Texture2D, listStairs[2].Rectangle, Color.White);
        }
        ...
        public void ClimbStairs() {
            ...
            {
                if ( Keyboard.GetState().IsKeyDown(Keys.Up) == true && Ecir.cameraMove.Intersects(listStairs[0].Rectangle) == true || Ecir.cameraMove.Intersects(listStairs[1].Rectangle) == true || Ecir.cameraMove.Intersects(listStairs[2].Rectangle) == true)
                {
                    climbStairs = true;
                }
                // else { climbStairs = false; }
                if (Ecir.cameraMove.Intersects(listStairs[0].Rectangle) == false && Ecir.cameraMove.Intersects(listStairs[1].Rectangle) == false && Ecir.cameraMove.Intersects(listStairs[2].Rectangle) == false) { climbStairs = false; }
            }
            else {climbStairs = false;}
